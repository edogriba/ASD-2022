...
void menuParola(void) {
    ...
    if ((fp = fopen("log.txt", "r")) == NULL) {
        ...
        struct tratta *vettore_pointer_fisso[n];
        struct tratta *vettore_pointer[ORDINAMENTI_RICHIESTI][n];
            ...
            switch (nomeComando) {
                ...
                case r_ricerca_lineare:
                    ...
                    MergeSortPartenza(vettore_pointer[2], vettore_copie,
                                      n); // Per usare la Binary Search prima devo ordinare il vettore per stazione di partenza
                    BinarySearch(vettore_pointer_fisso, string_tbf, n);
                    printf("\n");
                    break;
                case r_fine:
                    continua = 0;
