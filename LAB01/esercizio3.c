...
enum comando_e {r_contenuto_log=1, r_ordinamento_data_ora, r_ordinamento_codice_tratta, r_ordinamento_stazione_partenza, r_ordinamento_stazione_arrivo, r_ricerca_binaria, r_ricerca_lineare, r_fine};

void readData(FILE * fp, int n, struct tratta elenco_log[]);
...
void readData(FILE * fp, int n, struct tratta elenco_log[]) {
    int i;
    for (i=0; i<n; i++) {
        fscanf(fp, "GTT%s %s %s %s %s %s %d\n", elenco_log[i].codice_tratta, elenco_log[i].partenza, elenco_log[i].arrivo, elenco_log[i].data, elenco_log[i].ora_partenza, elenco_log[i].ora_arrivo, &elenco_log[i].ritardo);
    }
}
...
void menuParola(void) {
    ...
    if ((fp = fopen("log.txt", "r")) == NULL) {
        ...
        int continua = 1;
        fscanf(fp, "%d\n", &n);
        struct tratta vettore_struct[n];
        struct tratta *vettore_pointer[n];
        struct tratta *vettore_copie[n];
        struct tratta *elenco[ORDINAMENTI_RICHIESTI][n];
        char string_tbf[NMAX];
        readData(fp, n, vettore_struct);
        fclose(fp);
        for (i = 0; i < n; i++) {
            vettore_pointer[i] = &vettore_struct[i];
        }
        for (j = 0; j < ORDINAMENTI_RICHIESTI; j++) {
            for (k = 0; k < n; k++) {
                elenco[j][k] = &vettore_struct[k];
            }
        }
        ...
        while (continua) {
            ...
            switch (nomeComando) {
                ...
                case r_ricerca_binaria:
                    ...
                    LinearSearch(vettore_pointer, string_tbf, n);
                    printf("\n");
                    break;
                case r_ricerca_lineare:
                    ...
                    MergeSortPartenza(vettore_pointer, vettore_copie,
                                      n); // Per usare la Binary Search prima devo ordinare il vettore
                    BinarySearch(vettore_pointer, string_tbf, n);
                    printf("\n");
                    break;
                case r_fine:
                    continua = 0;
...
int leggiComando (void){
    ...
    char comandi_disponibili[NUMERO_COMANDI][MAX_STR+1] = {"contenuto_log", "ordinamento_data_ora", "ordinamento_codice_tratta", "ordinamento_stazione_partenza", "ordinamento_stazione_arrivo", "ricerca_binaria", "ricerca_lineare", "fine"};
    printf("Comando [contenuto_log/ordinamento_data_ora/ordinamento_codice_tratta/ordinamento_stazione_partenza/ordinamento_stazione_arrivo/ricerca_binaria/ricerca_lineare/fine] \n");
    scanf("%s", input);
